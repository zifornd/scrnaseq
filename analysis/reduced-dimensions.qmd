...
```{r}
hvg <- bplapply(sce, rowSubset, "HVG")
```
...
plt <- bpmapply(plotPCA, x = sce, name = names(sce), SIMPLIFY = FALSE)
...
plt <- bpmapply(plotTSNE, x = sce, name = names(sce), SIMPLIFY = FALSE)
...
plt <- bpmapply(plotUMAP, x = sce, name = names(sce), SIMPLIFY = FALSE)
...
out <- list(
  ...
  list(
    File = "",
    Description = "TSNE plot"
  ),
  list(
    File = "",
    Description = "UMAP plot"
  )
)

out <- jsonlite::toJSON(out, pretty = TRUE)

DT::datatable(jsonlite::fromJSON(out))
```
